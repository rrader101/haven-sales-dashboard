...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const {
      ...
      SALESFORCE_REPORT_ID_DAILY: REPORT_ID,
      SALESFORCE_API_VERSION
    } = process.env;

    if (!SALESFORCE_INSTANCE_URL || !REPORT_ID) {
      return res.status(500).json({ error: 'Salesforce instance URL or daily report ID not configured' });
    }
    ...
    if (!describeResp.ok) {
      ...
      return res
        .status(502)
        .json({ error: 'Error describing Salesforce daily report', details: text, status: describeResp.status });
    }

    const describeData = await describeResp.json();
    const metadata = describeData.reportMetadata;

    // Adjust upper-bound date filter similarly to weekly report
    ...
    if (!runResp.ok) {
      ...
      return res
        .status(502)
        .json({ error: 'Error running Salesforce daily report', details: text, status: runResp.status });
    }

    const reportData = await runResp.json();
    return res.status(200).json(reportData);
  } catch (err: any) {
    console.error('Daily Salesforce API error:', err);
    return res.status(500).json({ error: 'Unexpected Salesforce daily API error', details: err.message || String(err) });
  }
}
